...
from instance_manager import configure_root, create_mpack, get_conf_dir, get_log_dir, get_run_dir, list_instances, \
  set_mpack_instance
...
def init_action_parser(action, parser):
  ...
  try:
    ...
    parser.error(
      "Invalid action {0}. Possible options are: {1}".format(action, "|".join(action_parser_map)))


def init_common_parser_options(parser):
  parser.add_option('--root', default=os.environ.get("MPACK_INSTANCE_ROOT", "/usr/hwx"),
                    help="owned mpack instance root (default: %default)", dest="root")
...
def init_create_parser_options(parser):
  init_common_parser_options(parser)
...
def main(options, args):
  ...
  if action == CREATE_MPACK_INSTANCE_ACTION:
    ...
    result = list_instances(mpack=options.mpack, mpack_instance=options.mpack_instance,
                            subgroup_name=options.subgroup_name, module_name=options.module_name,
                            components_map=parsed_components_map)
...
if __name__ == "__main__":
  ...
  try:
    ...
    print("\nAborting ... Keyboard Interrupt.", file=sys.stderr)
    sys.exit(1)
