...
__all__ = ["configure_root", "create_mpack", "set_mpack_instance", "get_conf_dir",
           "get_log_dir", "get_run_dir", "list_instances", "walk_mpack_dict"]
...
DEFAULT_ROOT_FOLDER_PATH = "/usr/hwx"
ROOT_FOLDER_PATH = os.environ.get("MPACK_INSTANCE_ROOT", DEFAULT_ROOT_FOLDER_PATH)
...
def configure_root(root_path):
  """Configure the owned filesystem root used by this process."""
  if not root_path:
    raise ValueError("Instance root must not be empty")

  normalized_root = os.path.abspath(os.path.expanduser(root_path))
  if normalized_root == os.path.sep:
    raise ValueError("Instance root must not be the filesystem root")

  global ROOT_FOLDER_PATH
  ROOT_FOLDER_PATH = normalized_root


def validate_identifier(value, field_name, required=True):
  if value is None and not required:
    return
  if not isinstance(value, str) or not IDENTIFIER_PATTERN.fullmatch(value):
    raise ValueError("{0} contains an invalid path identifier".format(field_name))


def validate_component_selection(components, components_map, allow_default_filter=False):
  if components and components != "*":
    if not isinstance(components, (list, tuple)):
      raise ValueError("components must be a list or '*'")
    for component in components:
      validate_identifier(component, "component")

  if components_map is not None:
    if not isinstance(components_map, dict):
      raise ValueError("components-map must be a map")
    for component, instances in components_map.items():
      validate_identifier(component, "component")
      if not isinstance(instances, (list, tuple)):
        raise ValueError("component instances must be a list")
      for instance in instances:
        if not (allow_default_filter and instance == ""):
          validate_identifier(instance, "component instance")


def require_owned_path(path):
  root = os.path.realpath(ROOT_FOLDER_PATH)
  ...
  try:
    ...
    inside_root = False
  if not inside_root:
    raise ValueError("Path escapes the configured instance root: {0}".format(path))
  return path


def read_owned_link(link_path):
  """Read a symlink and return an absolute target confined to the owned root."""
  require_owned_path(link_path)
  if not os.path.islink(link_path):
    raise ValueError("Managed path is not a symlink: {0}".format(link_path))
  target = os.readlink(link_path)
  if not os.path.isabs(target):
    target = os.path.join(os.path.dirname(link_path), target)
  return require_owned_path(os.path.normpath(target))


configure_root(ROOT_FOLDER_PATH)
...
def create_mpack(mpack_name, mpack_version, mpack_instance, subgroup_name=DEFAULT_SUBGROUP_NAME, module_name=None,
  ...
  validate_identifier(mpack_name, "mpack")
  validate_identifier(mpack_version, "mpack version")
  validate_identifier(mpack_instance, "mpack instance")
  validate_identifier(subgroup_name, "subgroup")
  validate_identifier(module_name, "module")
  validate_component_selection(components, components_map)
...
def set_mpack_instance(mpack, mpack_version, mpack_instance, subgroup_name=DEFAULT_SUBGROUP_NAME, module_name=None,
  ...
  validate_identifier(mpack, "mpack")
  validate_identifier(mpack_version, "mpack version")
  validate_identifier(mpack_instance, "mpack instance")
  validate_identifier(subgroup_name, "subgroup")
  validate_identifier(module_name, "module")
...
def build_granular_json_with_filtering(mpack_name_filter, instance_name_filter, subgroup_name_filter,
  ...
  validate_identifier(mpack_name_filter, "mpack", required=False)
  validate_identifier(instance_name_filter, "mpack instance", required=False)
  validate_identifier(subgroup_name_filter, "subgroup", required=False)
  validate_identifier(module_name_filter, "module", required=False)
  validate_component_selection(None, components_name_filter_map, allow_default_filter=True)
...
def get_module_meta_mpack(path, module_name):
  ...
  if current_link_location is None:
    raise ValueError("No current version link exists below {0}".format(os.path.join(path, module_name)))
  current_target = read_owned_link(current_link_location)
...
def normalize_parameters(mpack_name, module_name, components, components_map):
  ...
  if components_map:
    components_map = dict((k.lower(), v) for k, v in components_map.items())
  ...
  def parse_mpack(path):
    ...
    try:
      ...
      raise ValueError("The {0} is invalid. Location: {1}. Error message: {2}".format(
        MPACK_JSON_FILE_NAME, mpack_json_path, str(e)))
...
class MpackInstance(Instance):
  ...
  def create_mpack_instance(mpack_name, mpack_version, mpack_instance, subgroup_name, module_name,
    ...
    elif not os.path.islink(default_mpack_instance_symlink):
      raise ValueError("Default mpack instance path is not a symlink: {0}".format(
        default_mpack_instance_symlink))
    else:
      read_owned_link(default_mpack_instance_symlink)
...
class ComponentInstance(Instance):
  ...
  def __init__(self, name, component_path, path_exec):
    ...
    self.path_exec = require_owned_path(path_exec)
    self.module_path = read_owned_link(self.path_exec)
  ...
  def set_new_version(self, mpack_name, mpack_version, component_type):
    mpack_path = require_owned_path(os.path.join(
      ROOT_FOLDER_PATH, MPACKS_FOLDER_NAME, mpack_name, mpack_version, component_type))
    ...
    if os.path.lexists(target_link):
      if not os.path.islink(target_link):
        raise ValueError("Component version path is not a symlink: {0}".format(target_link))
      if os.readlink(target_link) == mpack_path:
        return

    if not os.path.exists(mpack_path):
      raise ValueError("Path doesn't exist: " + mpack_path)

    temporary_link = "{0}.tmp-{1}".format(target_link, os.getpid())
    try:
      if os.path.lexists(temporary_link):
        os.remove(temporary_link)
      os.symlink(mpack_path, temporary_link)
      os.replace(temporary_link, target_link)
    finally:
      if os.path.lexists(temporary_link):
        os.remove(temporary_link)
  ...
  def create_component_instance(mpack_name, mpack_version, mpack_instance, subgroup_name, module_name,
    ...
    if is_client_module:
      ...
      component_path = os.path.join(ROOT_FOLDER_PATH, INSTANCES_FOLDER_NAME, mpack_name, mpack_instance, subgroup_name,
                                    module_name, component_type, component_instance_name)
    component_path = require_owned_path(component_path)
    mpack_path = require_owned_path(os.path.join(
      ROOT_FOLDER_PATH, MPACKS_FOLDER_NAME, mpack_name, mpack_version, component_type))
