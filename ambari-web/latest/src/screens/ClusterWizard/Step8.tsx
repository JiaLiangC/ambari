...
function Step8({ wizardName = "clusterCreation" }: Step8Props) {
  ...
  const completeProspectiveManagedDependencyPreviews = async (
    ...
    const pending = reviewedManagedDependencies(managedDependencies).filter(
      ({ choice, dependencyType }) => {
        if (!choice.preview?.compatible) return true;
        ...
        return !latestAttempt
          ...
          || Boolean(latestAttempt.request.draft
            && latestAttempt.request.draft.revision !== currentRevision);
      },
    );
    if (!pending.length) return;
    const { targetClusterName } = targetClusterIdentity();
    const nextSelections = cloneDeep(managedDependencies);
    let reviewChanged = false;
    for (const { choice, dependencyType } of pending) {
      if (!choice.provider) {
        throw new Error(t("installer.step8.dependencyProviderMissing"));
      }
      let preview: ManagedDependencyPreview;
      const record = managedDependencyMaterializations.current[dependencyType];
      try {
        preview = await ServiceDependenciesApi.previewService(
          targetClusterName,
          dependencyType,
          {
          ...
          undefined,
          choice.preview?.binding_id || record?.bindingId,
        );
      } catch (error: any) {
        assertCurrentDeployment(generation);
        if (record && isBindingIdUnavailable(error)) {
          ...
          if (reconciled) {
            if (!choice.preview
              || !managedDependencyAttemptMatchesPreview(reconciled.attempt, choice.preview)) {
              throw new Error(t("installer.step8.dependencyExistingBindingRequiresReview"));
            }
            const resolvedRecord = recordManagedDependencyResponse(
              record,
              reconciled.attempt,
              reconciled.binding,
            );
            await saveMaterializationRecord(dependencyType, resolvedRecord);
            completedOperationIds.current.add(
              `create-managed-dependency-${dependencyType}-${record.bindingId}`,
            );
            continue;
          }
        }
  ...
  const buildDeploymentStages = (generation: number) => {
    ...
    const managedDependencyOperations = reviewedManagedDependencies(managedDependencies)
      .map(({ choice, dependencyType }) => ({
        id: `create-managed-dependency-${dependencyType}-${choice.preview?.binding_id || "pending"}`,
        label: t("installer.step8.preparingManagedDependency", {
          dependency: t(`managedDependencies.${dependencyType.toLocaleLowerCase()}.label`),
        }),
        run: () => materializeManagedDependency(dependencyType, generation),
      }));
