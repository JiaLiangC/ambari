...
const mocks = vi.hoisted(() => ({
  ...
  previewManagedDependency: vi.fn(),
  print: vi.fn(),
...
vi.mock("../../api/serviceDependenciesApi", () => ({
  default: {
    create: mocks.createManagedDependency,
    get: mocks.getManagedDependency,
    previewService: mocks.previewManagedDependency,
  },
}));
...
describe("cluster deployment Review", () => {
  beforeEach(() => {
    ...
    mocks.previewManagedDependency.mockResolvedValue(managedHdfsPreview);
    mocks.createManagedDependency.mockImplementation(async (
      _clusterName: string,
  ...
  it("applies reviewed values, persists the exact create request, and pauses at provider preparation", async () => {
    ...
    expect(mocks.applyClusterConfigs.mock.invocationCallOrder[0])
      .toBeLessThan(mocks.previewManagedDependency.mock.invocationCallOrder[0]);
    expect(mocks.previewManagedDependency).toHaveBeenCalledWith(
      "cluster1",
      "HDFS",
      { cluster_id: 9, service_name: "HDFS" },
      undefined,
      BINDING_ID,
    );
    expect(mocks.createManagedDependency).toHaveBeenCalledWith(
      "cluster1",
      expect.objectContaining({
        binding_id: BINDING_ID,
        dependency_type: "HDFS",
    ...
    expect(mocks.dispatch.mock.invocationCallOrder[persistedRequestCall])
      .toBeLessThan(mocks.createManagedDependency.mock.invocationCallOrder[0]);
    expect(mocks.updateService).not.toHaveBeenCalled();
  });

  it("reconciles a lost binding response by exact binding and operation identity", async () => {
    mocks.createCluster.mockResolvedValue({ Clusters: { cluster_id: 72 } });
    mocks.createManagedDependency.mockRejectedValue(new Error("response lost"));
    ...
    expect(await screen.findByText(/Provider preparation has started/)).toBeTruthy();
    expect(mocks.createManagedDependency).toHaveBeenCalledOnce();
    expect(mocks.getManagedDependency).toHaveBeenCalledWith("cluster1", BINDING_ID);
    expect(mocks.previewManagedDependency).toHaveBeenCalledOnce();
    expect(mocks.updateService).not.toHaveBeenCalled();
  });
  ...
  it("reapproves a pre-POST attempt at the current draft revision without changing its binding", async () => {
    ...
    expect(await screen.findByText(/Provider preparation has started/)).toBeTruthy();
    expect(mocks.createManagedDependency).toHaveBeenCalledOnce();
    expect(mocks.createManagedDependency.mock.calls[0][1]).toMatchObject({
      binding_id: BINDING_ID,
      draft: { id: CLUSTER_DRAFT_ID, revision: expect.any(Number) },
      operation_id: OPERATION_ID,
    });
    expect(mocks.createManagedDependency.mock.calls[0][1].draft.revision)
      .toBeGreaterThan(1);
  ...
  it("reconciles the new attempt when CREATE wins after the preflight GET", async () => {
    ...
    mocks.createManagedDependency.mockRejectedValueOnce({
  ...
  it("shows an older winning attempt whose semantic fingerprints differ", async () => {
    ...
    mocks.previewManagedDependency.mockRejectedValueOnce({
    ...
    expect(mocks.createManagedDependency).not.toHaveBeenCalled();
    expect(mocks.updateService).not.toHaveBeenCalled();
  });
  ...
  it("retains same-workflow INIT HBase intent without creating or adopting another service", async () => {
    ...
    expect(mocks.createManagedDependency.mock.calls[0][1]).not.toHaveProperty("draft");
  });
