    ...
    if not name:
      ...
      if context.use_repos:
        ...
        if use_repos:
          ...
          for repo in use_repos:
            ...
            Logger.info(
              "Temporary sources file will be copied: {0}".format(new_sources_file)
            )
            sudo.copy(
      ...
      if is_tmp_dir_created:
        for temporary_sources_file in copied_sources_files:
          Logger.info(
            "Removing temporary sources file: {0}".format(temporary_sources_file)
          )
          os.remove(temporary_sources_file)
        if apt_sources_list_tmp_dir:
          Logger.info(
            "Removing temporary sources directory: {0}".format(apt_sources_list_tmp_dir)
          )
