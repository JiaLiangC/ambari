...
def download_file(link, destination, chunk_size=16 * 1024, progress_func=None):
  ...
  if os.path.exists(destination):
    print_warning_msg(
      "File {0} already exists, assuming it was downloaded before".format(destination)
    )
    return
...
def download_file_anyway(link, destination, chunk_size=16 * 1024, progress_func=None):
  print_info_msg(
    "Trying to download {0} to {1} with python lib [urllib2].".format(link, destination)
  )
  if os.path.exists(destination):
    print_warning_msg(
      "File {0} already exists, assuming it was downloaded before".format(destination)
  ...
  if not os.path.exists(destination):
    print(
      "Trying to download {0} to {1} with [curl] command.".format(link, destination)
    )
