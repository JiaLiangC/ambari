    ...
    if not is_stack_supports_ranger_kerberos or not is_security_enabled:
      ...
      if response_code is not None and response_code == 200:
        ...
        if user_resp_code is not None and user_resp_code == 200:
          ...
          while retryCount <= 5:
            ...
            if repo is not None:
              Logger.info(
                "{0} Repository {1} exist".format(component.title(), repo["name"])
              )
              break
            else:
              ...
              if response is not None:
                Logger.info(
                  "{0} Repository created in Ranger admin".format(component.title())
                )
                break
              else:
                if retryCount < 5:
      ...
      if response and response[0] == 200:
        ...
        while retryCount <= 5:
          ...
          if response is not None:
            Logger.info(
              "{0} Repository {1} exist".format(component.title(), (response["name"]))
            )
            break
          else:
