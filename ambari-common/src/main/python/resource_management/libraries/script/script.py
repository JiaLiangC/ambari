...
class Script(object):
  ...
  def post_start(self, env=None):
    ...
    for pid_file in pid_files:
      if not sudo.path_exists(pid_file):
        raise Fail(
          "Pid file {0} doesn't exist after starting of the component.".format(pid_file)
        )
  ...
  def get_package_from_available(self, name, available_packages_in_repos=None):
    ...
    package_regex = (
      name.replace(STACK_VERSION_PLACEHOLDER, "(\d|{0})+".format(package_delimiter))
      + "$"
    )
    ...
    if name_with_version:
      raise Fail(
        "No package found for {0}(expected name: {1})".format(name, name_with_version)
      )
    else:
      raise Fail(
    ...
    if componentCategory and componentCategory.strip().lower() == "CLIENT".lower():
      ...
      try:
        ...
        if service_name in services_to_skip:
          ...
          Logger.info(
            "Skipping status check for {0} service during upgrade".format(service_name)
          )
        else:
          self.status(env)
