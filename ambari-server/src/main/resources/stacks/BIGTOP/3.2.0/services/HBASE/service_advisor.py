...
class HBASERecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendHBASEConfigurationsFromHDP22(
    ...
    if int(clusterData["hbaseRam"]) > threshold:
      ...
      hbase_bucketcache_percentage_in_combinedcache_str = "{0:.4f}".format(
        math.ceil(hbase_bucketcache_percentage_in_combinedcache * 10000) / 10000.0
      )
  ...
  def recommendHBASEConfigurationsForKerberos(
    ...
    if is_kerberos_enabled:
      ...
      self.logger.debug(
        "Calculated Phoenix Query Server hosts: %s" % str(phoenix_query_server_hosts)
      )
      ...
      validationItems.append(
        {
          ...
          "item": self.getWarnItem("{0} should be float value".format(prop_name1)),
        }
      )
      ...
      validationItems.append(
        {
          ...
          "item": self.getWarnItem("{0} should be float value".format(prop_name2)),
        }
      )
    elif (
