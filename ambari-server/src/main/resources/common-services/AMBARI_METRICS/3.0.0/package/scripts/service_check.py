...
def call_curl_krb_request(
  ...
  for i in range(0, tries):
    try:
      ...
      if not response:
        Logger.error(
          "Unable to {0} metrics on: {1}.  Error: {2}".format(method, uri, errmsg)
        )
      else:
        Logger.info(
          "%s response from %s: %s, errmsg: %s" % (method, uri, response, errmsg)
        )
        try:
          response.close()
        except:
    ...
    if method == "GET":
      ...
      if not values_are_present:
        if i < tries - 1:  # range/xrange returns items from start to end-1
          Logger.info("Values weren't stored yet. Retrying in %s seconds." % (tries))
          time.sleep(connection_timeout)
        else:
          raise Fail(
            "Values %s and %s were not found in the response."
