...
def get_conf_dir():
  try:
    ...
    print_info_msg(
      "{0} is not set, using default {1}".format(AMBARI_CONF_VAR, default_conf_dir)
    )
...
def update_ambari_properties():
  ...
  try:
    ...
    print_error_msg(
      'Could not rename "%s" to "%s": %s' % (prev_conf_file, new_conf_file, str(e))
    )
...
class JDKRelease:
  ...
  def __load_properties(properties, section_name):
    if section_name is None or section_name == "":
      raise FatalException(
        -1,
        "Invalid properties section: " + ("(empty)" if section_name is None else ""),
      )
