...
def _uninstall_mpack(mpack_name, mpack_version):
    ...
    for dir in staged_mpack_dirs:
      ...
      if os.path.isdir(staged_mpack_dir):
        ...
        if (
          ...
          print_info_msg(
            "Removing management pack staging location {0}".format(staged_mpack_dir)
          )
  ...
  if not found:
    print_error_msg(
      "Management pack {0}-{1} is not installed!".format(mpack_name, mpack_version)
    )
  else:
    print_info_msg(
...
def _install_mpack(options, replay_mode=False, is_upgrade=False):
  ...
  if ambari_user:
    ...
    for pack in adjust_ownership_list:
      ...
      recursive = pack[3]
      logger.info(
        "Setting file permissions: {0} {1} {2} {3}".format(file, mod, user, recursive)
      )
...
def upgrade_mpack(options, replay_mode=False):
  ...
  print_info_msg(
    "Management pack {0}-{1} successfully upgraded!".format(mpack_name, mpack_version)
  )
  if not replay_mode:
    add_replay_log(
      UPGRADE_MPACK_ACTION,
    ...
    with open(replay_log_file, "r") as f:
      for replay_log in f:
        ...
        if replay_options.mpack_command == INSTALL_MPACK_ACTION:
          ...
          error_msg = "Invalid mpack command {0} in mpack replay log {1}!".format(
