...
public class ManagedDependencyOperationDispatcher {
  ...
  private void dispatchLocked(ServiceDependencyHostResultEntity supplied) throws AmbariException {
    ...
    if (!providerCommand && !isVerificationCommand(command.getCheckKind())) {
      throw new IllegalStateException("Only provider effects and strict verification are dispatchable");
    }
    ...
    Map<String, String> commandParameters = providerCommand
        ? Map.of(COMMAND_PARAMETER, command.getCommandJson())
        : Map.of(COMMAND_PARAMETER, command.getCommandJson(),
            ManagedDependencyRuntimePlanner.BUNDLE_PARAMETER,
            runtimePlanner.persistedPreparationBundle(
