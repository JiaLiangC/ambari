...
public class MpackDAO {
  ...
  public void removeCatalog(Long id) {
    ...
    Long retained = entityManager.createQuery("SELECT COUNT(r) FROM MpackTargetResourceEntity r WHERE (r.mpackId = :id OR r.materializedMpackId = :id) AND r.resourceState NOT IN ('PURGED', 'UNREGISTERED')",
    ...
    while (true) {
      List<MpackTargetResourceEntity> purged = entityManager.createQuery(
          "SELECT r FROM MpackTargetResourceEntity r WHERE (r.mpackId = :id OR r.materializedMpackId = :id) AND r.resourceState IN ('PURGED', 'UNREGISTERED') ORDER BY r.targetKey",
