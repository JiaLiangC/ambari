...
public class RegistryService extends BaseService {
  ...
  /** Updates a connected software registry after validating its document. */
  @PUT
  @Path("{registryId}")
  @Produces(MediaType.TEXT_PLAIN)
  @ApiOperation(value = "Updates a connected software registry")
  @ApiResponses({
    @ApiResponse(code = HttpStatus.SC_OK, message = MSG_SUCCESSFUL_OPERATION),
    @ApiResponse(code = HttpStatus.SC_BAD_REQUEST, message = MSG_INVALID_ARGUMENTS),
    @ApiResponse(code = HttpStatus.SC_NOT_FOUND, message = MSG_RESOURCE_NOT_FOUND),
  ...
  public Response updateRegistry(String body, @Context HttpHeaders headers, @Context UriInfo ui,
      @PathParam("registryId") String registryId) {
    return handleRequest(headers, body, ui, Request.Type.PUT, createRegistryResource(registryId));
  }

  /** Removes a registry that is not referenced by a registered mpack. */
  @DELETE
  @Path("{registryId}")
  @Produces(MediaType.TEXT_PLAIN)
  @ApiOperation(value = "Removes a connected software registry")
  @ApiResponses({
    @ApiResponse(code = HttpStatus.SC_OK, message = MSG_SUCCESSFUL_OPERATION),
    @ApiResponse(code = HttpStatus.SC_NOT_FOUND, message = MSG_RESOURCE_NOT_FOUND),
    @ApiResponse(code = HttpStatus.SC_CONFLICT, message = MSG_RESOURCE_ALREADY_EXISTS),
  ...
  public Response deleteRegistry(String body, @Context HttpHeaders headers, @Context UriInfo ui,
      @PathParam("registryId") String registryId) {
    return handleRequest(headers, body, ui, Request.Type.DELETE, createRegistryResource(registryId));
  }
