...
public class RegistryManagerImpl implements RegistryManager {
  ...
  private final Map<Long, Registry> registriesById = new ConcurrentHashMap<>();
  private final Map<String, Registry> registriesByName = new ConcurrentHashMap<>();

  @Inject
  public RegistryManagerImpl(RegistryDAO registryDAO, RegistryFactory registryFactory, MpackDAO mpackDAO) {
  ...
  void loadRegistries() {
    for (RegistryEntity registryEntity : registryDAO.findAll()) {
      try {
        ...
        LOG.error("Ignoring unsupported registry {} during startup", registryEntity.getRegistryId(), e);
      }
    }
  }
  ...
  public synchronized Registry addRegistry(String registryName, RegistryType registryType, String registryUri)
    ...
    if (registryName == null || registryName.trim().isEmpty()) {
      throw new AmbariException("Registry name must not be empty");
    }
    if (registryType == null) {
      throw new AmbariException("Registry type must not be empty");
    }
    RegistryEntity registryEntity = new RegistryEntity();
    registryEntity.setRegistryName(registryName.trim());
    registryEntity.setRegistryUri(registryUri);
    registryEntity.setRegistryType(registryType);
    if (registryDAO.findByName(registryEntity.getRegistryName()) != null) {
      throw new AmbariException("Registry " + registryEntity.getRegistryName() + " already exists");
    }
    ...
    if (registryId == null) {
      throw new AmbariException("Registry persistence did not return an identifier");
    }
  ...
  @Override
  public synchronized Registry updateRegistry(Long registryId, String registryName,
      RegistryType registryType, String registryUri) throws AmbariException {
    Registry current = getRegistry(registryId);
    validateIdentity(registryName, registryType);
    RegistryEntity duplicate = registryDAO.findByName(registryName.trim());
    if (duplicate != null && !registryId.equals(duplicate.getRegistryId())) {
      throw new AmbariException("Registry " + registryName.trim() + " already exists");
    }

    RegistryEntity candidate = new RegistryEntity();
    candidate.setRegistryId(registryId);
    candidate.setRegistryName(registryName.trim());
    candidate.setRegistryType(registryType);
    candidate.setRegistryUri(registryUri);
    Registry replacement = registryFactory.create(candidate);
    replacement.validate();
    registryDAO.merge(candidate);

    registriesById.put(registryId, replacement);
    registriesByName.remove(current.getRegistryName(), current);
    registriesByName.put(replacement.getRegistryName(), replacement);
    return replacement;
  }

  @Override
  public synchronized void removeRegistry(Long registryId) throws AmbariException {
    Registry registry = getRegistry(registryId);
    if (!mpackDAO.findByRegistryId(registryId).isEmpty()) {
      throw new AmbariException("Registry " + registryId + " is referenced by registered mpacks");
    }
    registryDAO.removeById(registryId);
    registriesById.remove(registryId, registry);
    registriesByName.remove(registry.getRegistryName(), registry);
  }
  ...
  public Map<Long, Registry> getRegistries() {
    return Collections.unmodifiableMap(new java.util.HashMap<>(registriesById));
  }

  private void validateIdentity(String registryName, RegistryType registryType) throws AmbariException {
    if (registryName == null || registryName.trim().isEmpty()) {
      throw new AmbariException("Registry name must not be empty");
    }
    if (registryType == null) {
