...
public class ManagedDependencyStackAdvisorPlannerTest {
  ...
  public void parsesActualFlattenedJsonShapeAndAuthorizesExactSelections() throws Exception {
    ...
    when(coordinator.authorizeAdvisorSelection(
        ConsumerReference.servicePlan(27, 9), ManagedDependencyType.HDFS,
        new ProviderReference(41, "HDFS"), HDFS_BINDING))
        .thenReturn(selection(HDFS_BINDING, ManagedDependencyType.HDFS, 41, "HDFS"));
    when(coordinator.authorizeAdvisorSelection(
        ConsumerReference.servicePlan(27, 9), ManagedDependencyType.ZOOKEEPER,
        new ProviderReference(42, "ZOOKEEPER"), ZK_BINDING))
        .thenReturn(selection(ZK_BINDING, ManagedDependencyType.ZOOKEEPER, 42, "ZOOKEEPER"));
  ...
  public void rejectsStaleFingerprintAndSchemaWithoutCreatingTrustedFacts() throws Exception {
    ...
    when(coordinator.authorizeAdvisorSelection(ConsumerReference.service(27),
        ManagedDependencyType.HDFS, new ProviderReference(41, "HDFS"), HDFS_BINDING))
        .thenReturn(changed);
