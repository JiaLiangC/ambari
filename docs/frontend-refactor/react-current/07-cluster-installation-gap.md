...
| 13 | `POST {stackVersionUrl}/recommendations` | Hosts, services, Blueprint, bindings, configuration properties, explicit numeric target for installed clusters, and an optional authoritative `managed_dependency_plan` | Step 5 checkpoints the safe provider selection, sends the exact schema-2 UUID and provider/consumer/snapshot fingerprints through both placement requests, and discards late draft responses. Step 6/7 and installed-service callers remain to be integrated. |
